...
Connection = module.exports = function Connection(config, def) {
    ...
    this.connect = function connect(cb) {
        ...
        akr.Connection.connect(this.config, function(err, akeraConn) {
            if (err) {
                cb(err);
            } else {
                self._CONNECTION = akeraConn;
                self._conn_write(akeraConn, cb);
                cb();
            }
        });
    };

    //queries the Akera Server
...
//private function for writing to a Akera Server
Connection.prototype._conn_write = function(akeraConn, cb) {
    var self = this;
    akeraConn.write(1, '', function(err) {
        if (err) {
            cb(err);
            return;
        } else {
            console.log('CONNECTED');
            self._conn_onMessage(akeraConn);
            self._conn_onEnd(akeraConn);
        }
    });
};
//private function for receiving message from a Akera Server
Connection.prototype._conn_onMessage = function(akeraConn) {
    var self = this;
    akeraConn.on('message', function(type, data) {
        if (!self._INIT)
            self._INIT = true;
        else {
...
//private function for terminating the connection to a Akera Server
Connection.prototype._conn_onEnd = function(akeraConn) {
    var self = this;
    akeraConn.on('end', function() {
